define([
    ...
    'use strict';
    /** 
     * @class widgets.brease.UserList.libs.Renderer
        ...
        if (!this.internal.filterSet) {
            ...
            $.fn.dataTable.ext.search.push(
                function (settings, data, dataIndex, row) {
                    var elem = $(settings.nTable).closest('[data-brease-widget="widgets/brease/UserList"]')[0];
    ...
    p._colorMeBlue = function () {
        ...
        for (var j = 0; j < children; j += 1) {
            for (var i = 0; i < this.widget.settings.config.style.length; i += 1) {
                ...
                if (rowEligible) {
                    this.widget.el.find('tbody').children('tr:eq(' + j + ')').addClass('widgets_brease_UserListStyle_style_style' + this.widget.settings.config.style[i].namePos);
                }
    ...
    p._fetchDataTableOptions = function () {
        ...
        options.order = (this.widget.settings.config.sort.length === 0) ? [[this.widget.settings.childrenIdList.length, 'asc']] : this.widget.settings.config.sort;
        return options;
    };
